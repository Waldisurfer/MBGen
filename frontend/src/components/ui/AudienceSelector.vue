...
const emit = defineEmits<{
  'update:modelValue': [value: Audience | null];
}>();
...
onMounted(async () => {
  if (!audienceStore.audiences.length) {
    await audienceStore.fetchAudiences();
  }
  // Auto-select latest if nothing selected yet
  if (!props.modelValue && audienceStore.latestAudience) {
    emit('update:modelValue', audienceStore.latestAudience);
  }
});

watch(() => audienceStore.latestAudience, (latest) => {
  if (!props.modelValue && latest) {
    emit('update:modelValue', latest);
  }
});

function select(audience: Audience) {
  emit('update:modelValue', audience);
  open.value = false;
}
  ...
  <div class="relative">
    ...
    >
      <span v-if="modelValue" class="flex items-center gap-2 min-w-0">
        ...
        <span class="flex gap-1 shrink-0">
          <span
            v-for="ch in modelValue.channels.slice(0, 3)"
            :key="ch"
            class="px-1.5 py-0.5 text-[10px] bg-gray-100 text-gray-500 rounded capitalize"
          >{{ ch }}</span>
        </span>
      </span>
      <span v-else class="text-gray-400">Select an audience…</span>
    ...
    >
      <div class="max-h-56 overflow-y-auto">
        ...
        <div v-if="audienceStore.isLoading" class="px-3 py-4 text-sm text-gray-400 text-center">
          Loading audiences…
        </div>

        <div v-else-if="!audienceStore.audiences.length" class="px-3 py-4 text-sm text-gray-400 text-center">
          No audiences yet.
          <RouterLink to="/audiences" class="text-brand-600 hover:underline ml-1" @click="open = false">
            Create one →
          </RouterLink>
        </div>
        ...
        <button
          ...
          :class="[
            ...
            modelValue?.id === audience.id ? 'bg-brand-50' : '',
          ]"
          @click="select(audience)"
        >
          <span class="flex-1 min-w-0">
            <span class="font-medium text-gray-900 block truncate">{{ audience.name }}</span>
            <span class="flex gap-1 mt-0.5">
              <span
                v-for="ch in audience.channels.slice(0, 4)"
                :key="ch"
                class="px-1.5 py-0.5 text-[10px] bg-gray-100 text-gray-500 rounded capitalize"
              >{{ ch }}</span>
            </span>
          </span>
          <svg v-if="modelValue?.id === audience.id" class="w-4 h-4 text-brand-600 shrink-0" fill="currentColor" viewBox="0 0 20 20">
